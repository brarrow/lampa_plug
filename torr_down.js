                    ...
                    case 8:
                        ...
                        _updateFileList = function updateFileList(sortType) {
                            ...
                            var newItems = sortedFiles.map(function(file) {
                                ...
                                var icon = '';
                                if (['mp4', 'mov', 'm4v'].includes(fileExt)) icon = '🎬';
                                else if (['mkv', 'avi'].includes(fileExt)) icon = '🎞️';
                                else if (['ts', 'mpg', 'mpeg'].includes(fileExt)) icon = '📺';
                                else icon = '🎥';
                                return {
                                    title: icon + ' ' + fileName,
...
>>>>>>> ea236e5 (first_edit)
      Lampa.Listener.follow('torrent', function (e) {
        if (e.type === 'onlong') {
          ...
          var labels = `${Lampa.Activity.active().movie?.first_air_date ? "tv" : "movie"}/${Lampa.Activity.active().movie?.id}`;
          var dtype = Lampa.Activity.active().movie?.first_air_date ? "TV" : "Movies";
          
          var onSelectApp = function(a) {
          ...
          // Universal клиент - добавляем обе опции
